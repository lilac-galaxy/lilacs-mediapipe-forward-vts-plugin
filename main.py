...
from vtube_studio_interface import get_authentication_token, vtube_studio_authenticate, send_detection_results
...
def get_args():
    ...
    parser.add_argument('-a','--auth_file', help="json file containing vtube studio auth token", default="auth.json")
    parser.add_argument('-m','--model',help="mediapipe model file", default="face_landmarker_v2_with_blendshapes.task")
...
def main(auth_token, args):
    ...
    with connect("ws://localhost:8001") as websocket:
        ...
        def process_results(detection_result : mp.tasks.vision.FaceLandmarkerResult, image : mp.Image, timestamp_ms : int):
        ...
        base_options = python.BaseOptions(model_asset_path=args.model, delegate=delagate)

        options = vision.FaceLandmarkerOptions(base_options,
