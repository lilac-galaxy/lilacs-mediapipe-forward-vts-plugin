...
def parameter_creation_request(parameter_name, explanation, min_val=0, max_val=1, default_val=0):
    message = {
...
def create_parameter(websocket, name, description, min_val, max_val, default_val):
        request_message_json = parameter_creation_request(name, description, min_val, max_val, default_val)
...
def create_custom_parameters(auth_token=""):
    ...
    with connect("ws://localhost:8001") as websocket:
        ...
        create_parameter(websocket, "lilac_MouthX", "mediapipe mouthX", min_val=-1, max_val=1, default_val=0)
        create_parameter(websocket, "lilac_BrowsLeftForm", "mediapipe mouthX", min_val=-1, max_val=1, default_val=0)
        create_parameter("lilac_BrowsRightForm", "mediapipe mouthX", min_val=-1, max_val=1, default_val=0)
        return auth_token
